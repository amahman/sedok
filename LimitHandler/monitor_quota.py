...
CHAT_ID_PATH = "/etc/rmbl/bot/notif/id"
BOT_KEY_PATH = "/etc/rmbl/bot/notif/key"

# Folder akun
PROTOCOLS = ["vless", "vmess", "trojan"]

# Log file
LOG_PATH = "/var/log/rmbl_quota_monitor.log"
...
def monitor():
    while True:
        ...
        for protocol in PROTOCOLS:
            usage_path = f"/etc/rmbl/{protocol}/usage"
            users = Path(usage_path).glob("*")
            for file in users:
                user = file.name
                limit_path = f"/etc/rmbl/{protocol}/usage/{user}"
                usage_file = f"/etc/limit/{protocol}/{user}"
                device_limit_path = f"/etc/rmbl/{protocol}/ip/{user}"
                ...
                try:
                    ...
                    if (limit > 0 and used > limit) or (max_devices > 0 and active_devices > max_devices):
                        ...
                        subprocess.run(["sed", "-i", f"/^### {user} /d", f"/etc/rmbl/{protocol}/.{protocol}.db"])

                        # Hapus file terkait
                        for f in [
                            f"/etc/limit/{protocol}/{user}",
                            f"/etc/limit/{protocol}/quota/{user}",
                            f"/etc/rmbl/{protocol}/usage/{user}",
                            f"/etc/rmbl/{protocol}/detail/{user}.txt",
